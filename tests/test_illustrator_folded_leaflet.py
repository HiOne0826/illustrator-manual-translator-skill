...
class FoldedLeafletPlanTests(unittest.TestCase):
    def test_reference_geometry_is_two_sides_of_five_76mm_panels(self):
        ...
        self.assertAlmostEqual(geometry["sectionGap"], 4.0)
        self.assertAlmostEqual(geometry["artboardGap"], 31.0)

    def test_non_section_cover_objects_keep_one_shared_shift(self):
        payload = inventory(1)
        ...
        self.assertFalse(first["sectionCompactionExpected"])
        self.assertEqual(first["layoutGroupCount"], 1)
    ...
    def test_title_bar_title_and_body_move_as_one_section(self):
        section_gap = folded.mm_to_pt(4)
        shifts, group_count, compacted, footer_anchored = folded._vertical_section_compaction(
            [0, 1, 2, 3, 4, 5, 6],
            {
        ...
        self.assertTrue(footer_anchored)
        ...
        first_bottom = -40 - (0 - (-150)) * 0.5 + shifts[2]
        second_top = -40 - (0 - (-250)) * 0.5 + shifts[3]
        self.assertAlmostEqual(first_bottom - second_top, section_gap)
    ...
    def test_qa_allows_natural_bottom_whitespace_for_one_short_section(self):
        issues = folded.validate_qa({
            ...
            "panelContentMetrics": [{
                "targetArtboard": 1, "targetPanel": 2, "layoutGroupCount": 1,
                "sectionCompactionExpected": False,
                "topBlankPt": folded.mm_to_pt(5.8), "bottomBlankPt": folded.mm_to_pt(60),
            }],
        }, folded.normalize_geometry())
        self.assertEqual(issues, [])

    def test_qa_does_not_fill_bottom_whitespace_by_spreading_sections(self):
        issues = folded.validate_qa({
            "artboardCount": 2,
            "panelCountPerSide": 5,
            "mediaWidthPt": folded.mm_to_pt(390),
            ...
            "panelContentMetrics": [{
                ...
                "sectionCompactionExpected": True,
    ...
    def test_jsx_uses_native_objects_and_creates_print_marks(self):
        ...
        self.assertIn("sectionCompactionExpected", jsx)
        self.assertIn("footerAnchored", jsx)
