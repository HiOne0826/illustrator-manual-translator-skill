...
The implementation scales source objects uniformly to the panel width, then identifies complete visual sections. A full-width filled title bar starts a section; the bar, title, corresponding body, and associated image move with one shared offset. Body sections flow sequentially from the reference top inset with the configured `sectionGap` (`4 mm` by default). Only a detected page-number footer forms a separate block and aligns to the bottom inset. Cover artwork without title bars remains one group so product images, contact blocks, and branding keep their original relationships. Object dimensions and every internal section offset remain proportional: titles and bodies are never spread independently, complete body sections are never pushed to the bottom merely to fill height, and images or text are not stretched vertically. A short panel may retain unavoidable bottom whitespace because the module does not duplicate or invent content. It does not place a PDF page or rasterize the source. Text below the configured `minimumBodyFontPt` is raised to the minimum and any resulting overset TextFrame is blocking.

QA records `panelContentMetrics` and blocks when whitespace before the first body block exceeds `12 mm`. Bottom whitespace is reported but is never removed by spreading sections; matching a denser reference requires a panel-level editorial remap or additional content.
