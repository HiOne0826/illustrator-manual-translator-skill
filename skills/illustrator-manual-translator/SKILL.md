...
- Five-fold AI/PDF must preserve native editable objects, use two `390 × 174.6 mm` artboards with five `76 × 156.22 mm` finished panels per side, preserve title/body relative spacing while aligning complete sections to the reference top inset, keep body text at or above the configured minimum, and block on overset text or excessive whitespace before a section. Never spread internal objects merely to fill panel height.
