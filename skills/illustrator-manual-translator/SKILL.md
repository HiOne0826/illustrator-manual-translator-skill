...
- Treat small-format pagination and print imposition as separate stages. `layout-small-format` creates a variable number of `76 × 156.22 mm` pages from content capacity; never create or split pages merely to reach ten panels. Keep all fitting sections from one source half on the same small page and greedily merge adjacent sparse, footer-free source halves when they fit.
- Treat five-fold output only as an optional imposition after the small-page content layout is accepted. Require an explicit panel mapping and confirmed duplex flip direction; never let fold geometry decide content pagination.
- Five-fold AI/PDF must preserve native editable objects and use two `390 × 174.6 mm` artboards with five `76 × 156.22 mm` finished panels per side. Treat a title bar, its title, and corresponding body/images as one section whose internal spacing cannot change. Flow body sections sequentially from the reference top inset with the configured section gap; only a page-number footer may align independently to the bottom inset. Keep body text at or above the configured minimum and block on overset text or excessive top whitespace. Never spread objects or complete body sections merely to fill panel height; unresolved bottom whitespace requires editorial remapping or more content.
...
For a small-format manual, run `layout-small-format` after electronic confirmation. Its page count is content-driven, not fixed. Show every generated small page and wait for explicit confirmation before `confirm-small-format` creates delivery. Only if the customer separately requests a folded print sheet should the confirmed small pages continue to an explicit imposition step such as five-fold.
