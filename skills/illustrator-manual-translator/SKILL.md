...
- Five-fold AI/PDF must preserve native editable objects and use two `390 × 174.6 mm` artboards with five `76 × 156.22 mm` finished panels per side. Treat a title bar, its title, and corresponding body/images as one section whose internal spacing cannot change; distribute only complete sections and page-number footers between the reference top and bottom insets. Keep body text at or above the configured minimum and block on overset text, excessive top whitespace, or excessive bottom whitespace when a panel has multiple distributable sections. Never spread objects inside a section merely to fill panel height.
